...
# Tool for downloading files and extracting compressed
# archives. Currently this is console-based  but in the
# future will have SOC for any interface.
#
# TODO: Should thre be two classes? The main Downloader which is
# independent of any interface. The second "Console::Downloader"
# using the first but with a console interface?
#
# == DESCRIPTION
# 
# Utility for downloading files and extracting
# archive files. Currently this is console-based
# but in the future will have SOC for any interface.
# 
# It also a rather old library and is likely in need
# of some "dusting off".
# 
# 
# == FEATURES/ISSUES
# 
# * Provide progressbar tracking.
# * Able to attempt download from fastest of a set of mirrors.
# * Decompression is currently shell based.
#
# Downloader requires either the Progressbar by ___ (gem install progressbar)
# or Clio by yours truly (gem install clio). One of these must be installed separately.
...
class Downloader
  ...
  #
  def unbracket(str) #bra=nil, ket=nil)
    case str
    when /^\[(.*?)\]$/, /^\{(.*?)\}$/, /^\<(.*?)\>$/, /^\((.*?)\)$/
      str.slice(1...-1)
    else
